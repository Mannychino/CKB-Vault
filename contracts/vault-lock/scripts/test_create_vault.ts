...
async function main() {
  ...
  const vaultDep = ccc.OutPoint.from({
  ...
  const tx = ccc.Transaction.from({
    outputs: [
      {
        capacity: ccc.fixedPointFrom(VAULT_AMOUNT),
        lock: vaultLock,
      },
    ],
  ...
  console.log(
    `Inputs selected: ${tx.inputs.length}`,
  );
  ...
  console.log(
    `Final inputs: ${tx.inputs.length}`,
  );

  console.log(
    `Final outputs: ${tx.outputs.length}`,
  );

  console.log(
    `Cell deps: ${tx.cellDeps.length}`,
  );
  ...
  tx.inputs.forEach((input, i) => {
    console.log(
      `${i}: ${input.previousOutput.txHash}:${input.previousOutput.index}`,
    );
  });
  ...
  tx.cellDeps.forEach((dep, i) => {
    console.log(
      `${i}: ${dep.outPoint.txHash}:${dep.outPoint.index}`,
      `depType=${dep.depType}`,
    );
  });
  ...
  tx.outputs.forEach((output, i) => {
    console.log(
      `${i}: ${ccc.fixedPointToString(output.capacity)} CKB`,
    );
  });
