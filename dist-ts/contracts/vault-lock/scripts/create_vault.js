...
const FUNDING_TX_HASH = "0x1bb87da347a776a927ab6593e1e10304ca195f8e24279f039008d5e3115b1bf7";
const FUNDING_INDEX = 22n;
// Expected funding capacity:
// 42,000,000 CKB = 4,200,000,000,000,000 Shannons
const FUNDING_CAPACITY = core_1.ccc.fixedPointFrom("42000000");
// --------------------------------------------------
// Vault parameters
// --------------------------------------------------
...
async function main() {
    ...
    const fundingOutPoint = core_1.ccc.OutPoint.from({
        txHash: FUNDING_TX_HASH,
        index: FUNDING_INDEX,
    });
    const fundingCell = await client.getCell(fundingOutPoint);
    if (!fundingCell) {
        throw new Error(`Funding Cell is not live: ${FUNDING_TX_HASH}:${FUNDING_INDEX}`);
    }
    console.log("Funding Cell found.");
    console.log("Funding capacity:", fundingCell.cellOutput.capacity.toString(), "Shannons");
    if (fundingCell.cellOutput.capacity !== FUNDING_CAPACITY) {
        throw new Error(`Unexpected funding capacity. Expected ${FUNDING_CAPACITY.toString()} Shannons, got ${fundingCell.cellOutput.capacity.toString()} Shannons.`);
    }
    ...
    if (!vaultDepCell) {
        throw new Error(`Vault contract Cell is not live: ${VAULT_LOCK_DEP_TX_HASH}:${VAULT_LOCK_DEP_INDEX}`);
    }
    console.log("Vault contract Cell found.");
    console.log("Contract capacity:", vaultDepCell.cellOutput.capacity.toString(), "Shannons");
    // --------------------------------------------------
    // Create vault lock
    // --------------------------------------------------
    ...
    const [addedInputs, hasChange] = await tx.completeFeeBy(signer, undefined, undefined, {
    ...
    for (let i = 0; i < tx.outputs.length; i++) {
        console.log(`Output ${i} capacity:`, tx.outputs[i].capacity.toString(), "Shannons");
    }
    // --------------------------------------------------
    // Send transaction
    // --------------------------------------------------
    ...
    tx.inputs.forEach((input, i) => {
        console.log(`Input ${i}:`, `${input.previousOutput.txHash}:${input.previousOutput.index}`);
    });
    console.log("\nSending transaction...");
    console.log("\nCellDeps before send:");
    tx.cellDeps.forEach((dep, i) => {
    ...
    const vaultCell = await client.getCell(core_1.ccc.OutPoint.from({
        txHash,
        index: 0n,
    }));
    if (!vaultCell) {
        throw new Error(`Could not resolve newly created Vault Cell: ${txHash}:0`);
    }
    console.log("\n========================================");
    console.log("VAULT CELL CREATED");
    console.log("========================================");
    console.log("OutPoint:", `${txHash}:0`);
    console.log("Capacity:", vaultCell.cellOutput.capacity.toString(), "Shannons");
